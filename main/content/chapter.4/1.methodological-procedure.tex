...
\begin{enumerate}
      ...
      \item El sistema debe contar con una característica de inicio automático después de cualquier corte de energía o reinicio.
...
\begin{figure}[ht!]
      ...
      \caption{Diagrama de arquitectura de sistema con ESP32 y detección de intensidad de sonido}
...
\begin{figure}[ht!]
      ...
      \caption{Diagrama de arquitectura de sistema con ESP32, micrófono de señales de audio y prophet}
...
\begin{figure}[ht!]
      ...
      \caption{Diagrama de arquitectura de sistema con Raspberry Pi, Yamnet y Cadenas de Markov}
...
\begin{figure}[ht!]
      \centering
      \includegraphics[width=0.85\textwidth]{Arquitectura/I5.png}
      \caption{Diagrama de arquitectura de sistema con Raspberry Pi Autonomo, Yamnet y Modelos de IA para detección de anomalías}
      \label{fig:prototipo5}
\end{figure}

\usubsection{Quinta Iteración}

En esta quinta iteración, sustituimos el análisis predictivo por uno descriptivo para el desarrollo de la solución de detección de anomalías, como respuesta a las limitaciones encontradas con las cadenas de Markov. Para ello, basamos nuestro enfoque en el trabajo de \citeauthor{reis2025edge} \citeyear{reis2025edge}, cuya investigación se centra en arquitecturas de Inteligencia Artificial en el borde (Edge AI) para el monitoreo en tiempo real. Específicamente, su estudio valida el uso de dos tecnologías que adoptamos: Isolation Forest (IF), un modelo para la detección de eventos anomalos basado en árboles, y un (LSTM) Autoencoder, una red neuronal para el análisis de patrones secuenciales complejos. En la figura \ref{fig:prototipo5} se ilustra la arquitectura del sistema implementado en esta iteración.
...
En el caso del Isolation Forest, adoptamos los hiperparámetros del estudio, configurando el modelo con 100 árboles ($n_estimators$) y un parámetro de contaminación del 3\%. Este último valor asume una proporción esperada de anomalías en nuestros datos, alineándose con los escenarios simulados en la investigación de referencia, Véase en la figura \ref{fig:isolation-forest-algorithm}
...
\begin{enumerate}
      \item Preparación de los Datos:
            ...
            De manera similar, el modelo puede procesar etiquetas de texto como ``uso de microondas'' o ``sensor de movimiento''. Pero le facilitaría el aprendizaje si estas etiquetas se convierten en números. Un error común sería asignarles un número a cada una (ej: microondas=1, sensor=2), ya que esto crearía una falsa relación matemática (como si el ``sensor'' fuera el doble del ``microondas'').
            ...
            Para la construcción del dataset de entrenamiento, se adopto la metodología de Reis et al. \citeyear{reis2025edge}, que consiste en la construccion de un dataset sintético basado en la simulación de secuencias de eventos normales y anómalos. Se generaron múltiples secuencias de eventos que reflejan el comportamiento típico del entorno doméstico, incorporando variaciones para simular anomalias dentro del conjunto de datos. Los datos se conformaban por una marca temporal y una etiqueta de evento, que luego fueron procesados con las tecnicas descritas anteriormente generando un csv con las características numericas descritas anteriormente.

            Se configuro una ventana deslizante para generar estas secuencias, donde cada nueva secuencia comenzaba un evento después de la anterior, asegurando así que el modelo pudiera aprender las transiciones entre eventos. Este enfoque permitió crear un conjunto de datos robusto y representativo para entrenar el modelo LSTM Autoencoder.

            El dataset de entrada previo a la transformación se ilustra en la figura \ref{fig:dataset-lstm}.

            \begin{figure}[ht!]
                  \centering
                  \includegraphics[width=0.35\textwidth]{LSTM/Screenshot_2.png}
                  \caption{Visualización del dataset de secuencias para el entrenamiento de los Modelos}
                  \label{fig:dataset-lstm}
            \end{figure}
...
\myequation{Accuracy = \frac{TP+TN}{TP+TN+FP+FN}=\frac{(18+45814)}{(18+45814+3+6)}\approx 0.9998}{Exactitud para LSTM Autoencoder.}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}

\myequation{Error = \frac{FP+FN}{TP+TN+FP+FN}=\frac{(3+6)}{(18+45814+3+6)}\approx 0.0002}{Tasa de Error para LSTM Autoencoder.}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}

\myequation{Recall = \frac{TP}{TP+FN}=\frac{(18)}{(18+6)}=0.75}{Sensibilidad para LSTM Autoencoder.}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}

\myequation{Precision = \frac{TP}{TP+FP}=\frac{18}{18+3}\approx 0.8571}{Precisión para LSTM Autoencoder.}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}

\myequation{Specificity = \frac{TN}{TN+FP}=\frac{45814}{45814+3}\approx 0.9999}{Especificidad para LSTM Autoencoder.}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}

\myequation{F1_{score} = 2 \cdot \frac{Precision \cdot Recall}{Precision + Recall} = 2 \cdot \frac{0.8571 * 0.75}{0.8571+0.75} \approx 0.8}{F1 Score para LSTM Autoencoder.}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}
...
\begin{itemize}
      ...
      \myequation{Accuracy = \frac{(2877+87737)}{(2877+87737+2164+8031)}\approx 0.8989}{Exactitud para Isolation Forest}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}

      \myequation{Error = \frac{(2164+8031)}{(2877+87737+2164+8031)}\approx 0.101}{Tasa de Error para Isolation Forest}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}

      \myequation{Recall = \frac{2877}{2877+8031}\approx 0.2638}{Sensibilidad para Isolation Forest}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}

      \myequation{Precision = \frac{2877}{2877+2164}\approx 0.5707}{Precisión para Isolation Forest}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}

      \myequation{Specificity = \frac{87737}{87737+2164}\approx 0.9759}{Especificidad para Isolation Forest}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.} 
      \myequation{F1_{score} = 2 \cdot \frac{(0.5707 * 0.2638)}{(0.5707 + 0.2638)}\approx 0.361}{F1 Score para Isolation Forest}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}
...
\begin{figure}[H]
      \centering
      \includegraphics[width=0.50\textwidth]{LSTM/roc_optimized.png}
...
\begin{figure}[H]
      \centering
      \includegraphics[width=0.50\textwidth]{LSTM/animalias_optimized.png}
...
\myequation{Accuracy = \frac{(19+45820)}{(19+45820+2+0)}\approx 0.9999}{Exactitud para LSTM Autoencoder V2}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}
\myequation{Error = \frac{(2+0)}{(19+45820+2+0)}\approx 0.00004}{Tasa de Error para LSTM Autoencoder V2}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}
\myequation{Recall = \frac{19}{19+0}\approx 1.0000}{Sensibilidad para LSTM Autoencoder V2}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}
\myequation{Precision = \frac{19}{19+2}\approx 0.9048}{Precisión para LSTM Autoencoder V2}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}
\myequation{Specificity = \frac{45820}{45820+2}\approx 0.9999}{Especificidad para LSTM Autoencoder V2}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}
\myequation{F1_{score} = 2 \cdot \frac{(0.9048 * 1.0000)}{(0.9048 + 1.0000)}\approx 0.95}{F1 Score para LSTM Autoencoder V2}{Hand et al. (2020). \textit{An Introduction to the Principles of Data Science}. Springer.}
...
\begin{figure}[H]
      \centering
      \includegraphics[width=0.65\textwidth]{Arquitectura/Arquitectura3.png}
...
\begin{figure}
      \centering
      \includegraphics[width=0.85\textwidth]{notifications/notificaciones1.jpg}
      \caption{Ejemplo de notificaciones.}
      \label{fig:notificaciones}
\end{figure}

La figura \ref{fig:notificaciones} muestra ejemplos de notificaciones enviadas por el sistema a través de Telegram, ilustrando cómo se comunica la información crítica al usuario final.

El sistema final implementado se configura como una solución de Edge Computing autónoma que opera en el dispositivo Raspberry Pi. El flujo de trabajo comienza con la \textbf{Captación de Audio} a través de micrófonos conectados, garantizando la adquisición continua de la señal acústica. Estos datos brutos son inmediatamente procesados por la \textbf{Capa de Clasificación}, que utiliza agentes como YAMNet y Vosk para \textbf{seleccionar y transformar} el audio en eventos categóricos y representaciones semánticas (datos característicos). Esta información estructurada alimenta el \textbf{LSTM Autoencoder V2}, el modelo central de la \textbf{Capa de Detección de Anomalías}, el cual ha sido optimizado bayesianamente para calcular el error de reconstrucción en tiempo real. Finalmente, si el error excede el umbral redefinido, la \textbf{Capa de Gestión de Alertas} valida la situación, y la \textbf{Capa de Notificaciones} genera el \textbf{resultado obtenido} mediante un mensaje de emergencia a través de Telegram o SMTP. De este modo, el sistema ofrece un mecanismo de monitoreo acústico autonomo, capaz de detectar y notificar eventos anómalos en tiempo real, todo dentro del entorno local del dispositivo edge.
